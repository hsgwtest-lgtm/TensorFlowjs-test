...
(function () {
  ...
  function render() {
    ...
    for (const nest of nests) {
      ...
      // ─── Reproduction indicator: food bar + queen crown ───
      // Food bar above nest
      const barW = 24;
      const barH = 3;
      const foodRatio = Math.min(1, nest.food / 30);
      ctx.fillStyle = 'rgba(0,0,0,0.4)';
      ctx.fillRect(nest.x - barW / 2, nest.y - 20, barW, barH);
      // Color changes based on enough food to reproduce
      const barColor = nest.food >= 2 ? `rgb(${sc[0]},${sc[1]},${sc[2]})` : 'rgba(150,150,150,0.5)';
      ctx.fillStyle = barColor;
      ctx.fillRect(nest.x - barW / 2, nest.y - 20, barW * foodRatio, barH);
      // Border
      ctx.strokeStyle = 'rgba(200,170,100,0.3)';
      ctx.lineWidth = 0.5;
      ctx.strokeRect(nest.x - barW / 2, nest.y - 20, barW, barH);

      // Queen crown icon
      ctx.fillStyle = `rgba(${sc[0]},${sc[1]},${sc[2]},0.9)`;
      ctx.font = '6px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText('👑', nest.x, nest.y - 23);

      // Population label at nest
      ctx.fillStyle = `rgba(${sc[0]},${sc[1]},${sc[2]},0.8)`;
      ctx.font = '7px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(`${nest.population}匹`, nest.x, nest.y + 20);
    }
